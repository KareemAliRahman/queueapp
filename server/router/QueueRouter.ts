...
QueueRouter.get("/", async (req, res, _next) => {
  ...
  try {
    ...
    res
      .status(404)
      .json({ error: error, message: "failed to get all queues of admin" });
  }
});
...
QueueRouter.post("/", async (req, res) => {
  try {
    ...
    await queueRepo.createNewQueue(
      req.body.name,
      user,
    ...
    res
      .status(403)
      .json({ error: error, message: "failed to create new queue" });
  }
});
...
QueueRouter.delete("/", async (req, res) => {
  try {
    ...
    if (queue.admin.id !== user.id) {
      return res
        .status(401)
        .json({ message: "queue admin only can delete a queue" });
    }
...
QueueRouter.post("/members", async (req, res) => {
  try {
    ...
    res
      .status(403)
      .json({ error: err.message, message: "failed to add member to queue" });
  }
});
